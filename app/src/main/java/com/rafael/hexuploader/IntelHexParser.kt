...
object IntelHexParser {
    ...
    // Todo registro Intel HEX tem no mínimo: byteCount + addrHigh + addrLow
    // + recordType + checksum = 5 bytes, mesmo quando byteCount é 0.
    private const val MIN_RECORD_BYTES = 5
    ...
    private fun hexLineToBytes(line: String): IntArray {
        ...
        for (i in bytes.indices) {
            ...
            bytes[i] = hex.toIntOrNull(16)
                ?: throw HexParseException("Caractere hexadecimal inválido na linha: $line")
        }
        // Todo registro precisa ter pelo menos byteCount+addrH+addrL+type+checksum.
        if (bytes.size < MIN_RECORD_BYTES) {
            throw HexParseException("Linha .hex incompleta: $line")
        }
