    ...
    private fun doUpload() {
        ...
        Thread {
            ...
            try {
                ...
                runOnUiThread {
                    // Se a Activity já não estiver mais visível quando essa
                    // lambda rodar, nem tenta mostrar o diálogo — evita
                    // WindowManager.BadTokenException.
                    if (isFinishing || isDestroyed) {
                        confirmQueue.offer(false)
                    } else {
                        AlertDialog.Builder(this)
                            .setTitle("Chip detectado")
                            .setMessage("$chipName\n\nContinuar com a gravação?")
                            .setCancelable(false)
                            .setPositiveButton("Gravar") { _, _ -> confirmQueue.offer(true) }
                            .setNegativeButton("Cancelar") { _, _ -> confirmQueue.offer(false) }
                            .show()
                    }
                }

                // Espera a resposta com timeout curto e repetido, em vez de
                // confirmQueue.take() (bloqueio indefinido). Assim, se a
                // Activity for destruída com o diálogo pendente (usuário troca
                // de app, sistema mata a Activity em background etc.), a
                // thread de gravação percebe isso e aborta em vez de travar
                // para sempre.
                var confirmed = false
                var answered = false
                while (!answered) {
                    ...
                    val result = confirmQueue.poll(300, TimeUnit.MILLISECONDS)
                    if (result != null) {
                        confirmed = result
                        answered = true
                    }
